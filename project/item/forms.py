...
class AddNewItem(FlaskForm):
    ...
    item_price = FloatField("Price", validators=[DataRequired(), NumberRange(min=0, max=10000000)], default=0.0)
    description = StringField("Description", validators=[DataRequired(), Length(min=5)], default="")
    inventory = IntegerField("Inventory", validators=[DataRequired(), NumberRange(min=0, max=10000000)], default=0)
